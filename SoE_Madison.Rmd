...
morphology <- read_csv("/users/aperium/Dropbox/Projects/Chenopodium/Data/Bisected seeds/bisectedseeds.csv") %>%
  ...
  mutate(seed_category = factor(seed_category, levels = unique(seed_category[order(case_when(
    seed_category == "album"         ~ 6,
    seed_category == "black"         ~ 5,
    seed_category == "lightred"      ~ 7,
    seed_category == "darkred"       ~ 7,
    seed_category == "red"           ~ 4,
...
lm(outer_epiderm_mean ~ seed_category, data = morphology) %>% summary()
aov(outer_epiderm_mean ~ seed_category, data = morphology) %>% TukeyHSD()


lm(diameter_mean ~ seed_category, data = morphology) %>% summary()
...
alpha_a <- ggplot(morphology, aes(x = seed_category, y = outer_epiderm_mean, fill = seed_category, color = seed_category)) +
...
alpha_b <- ggplot(morphology, aes(x = seed_category, y = volume, fill = seed_category)) +
...
alpha_c <- ggplot(morphology, aes(x = seed_category, y = pmax(length, width)/1000, fill = seed_category, color = seed_category)) +
...
alpha_d <- ggplot(morphology, aes(x = seed_category, y = (length + width)/2000, fill = seed_category, color = seed_category)) +
  geom_hline(yintercept = 1.5, color = outline_color, linetype = 3, size = 1) +
  geom_boxplot(show.legend = FALSE, size = 2*11/7, width = 2/3) +
...
#Red and black do not have different responses to stratification (Adjusted R-squared:  0.03265, F-statistic: 2.114 on 3 and 96 DF,  p-value: 0.1035)
rd_adj5 %>%
  filter(color == "black" | color == "red") %>%
  lm(adjusted_germination ~ treatment * color, data = .) %>% summary()
...
rd_adj5 %>%
  filter(color == "black") %>%
  lm(adjusted_germination ~ treatment, data = .) %>% summary()
...
#   scale_color_manual(values = c("bl"="grey30", "rd"="firebrick3", "nuttalliae" = "gold")) +
#   scale_fill_manual(values = c("bl"="grey30", "rd"="firebrick3", "nuttalliae" = "gold")) +
#   scale_x_continuous(breaks = c(0,1,2,4,6,9), minor_breaks = c(1*0:9)) +
#   theme_minimal()
...
ggplot(aes(x = treatment, y = adjusted_germination, color = color, fill = color)) +
  # geom_jitter(width = 0.1) +
  geom_smooth(method = "lm", formula = y ~ x) +
  geom_boxplot(data = rd_adj5 %>% filter(treatment==0), fill = NA, size = 2)+
  geom_boxplot(data = rd_adj5 %>% filter(treatment==1), fill = NA, size = 2)+
  geom_boxplot(data = rd_adj5 %>% filter(treatment==2), fill = NA, size = 2)+
  geom_boxplot(data = rd_adj5 %>% filter(treatment==4), fill = NA, size = 2)+
  geom_boxplot(data = rd_adj5 %>% filter(treatment==6), fill = NA, size = 2)+
  geom_boxplot(data = rd_adj5 %>% filter(treatment==9), fill = NA, size = 2)+
  scale_color_manual(values = fill_color3) +
  scale_fill_manual(values = fill_color3) +
  scale_x_continuous(breaks = c(0,1,2,4,6,9), minor_breaks = c(1*0:9)) +
...
#   # geom_jitter(width = 0.1) +
#   geom_smooth(method = "lm", formula = y ~ x) +
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==0), fill = NA)+
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==1), fill = NA)+
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==2), fill = NA)+
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==4), fill = NA)+
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==6), fill = NA)+
#   geom_boxplot(data = rd_adj5 %>% filter(treatment==9), fill = NA)+
#   scale_color_manual(values = c("bl"="grey30", "rd"="firebrick3", "nuttalliae" = "gold")) +
#   scale_fill_manual(values = c("bl"="grey30", "rd"="firebrick3", "nuttalliae" = "gold")) +
#   scale_x_continuous(breaks = c(0,1,2,4,6,9), minor_breaks = c(1*0:9)) +
#   theme_minimal()
