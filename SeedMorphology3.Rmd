...
morphology_LDB <- morphology %>%
  filter(seed_category == "lightred" | seed_category == "darkred" | seed_category == "black")


morphology_trimmed_for_PCA_LDB <- morphology_LDB  %>%
  mutate(seed_category = ifelse(species == "berlandieri", color, species)) %>%  
  mutate(seed_category = factor(seed_category, levels = unique(seed_category[order(case_when(
    seed_category == "black"         ~ 3,
    seed_category == "lightred"      ~ 1,
    seed_category == "darkred"       ~ 2
  ))]))) %>%
...
morphology_LDB.S <- morphology_trimmed_for_PCA_LDB %>%
  dplyr::select_if(is_type_numeric) %>%
  dplyr::select( -id_number, -seed_category ) %>%
  rda(scale=T, na.action = na.omit)

morphology_LDB.PCA <- scores(morphology_LDB.S, choices=c(1:3), display="wa", scaling = "species")

ev1_LDB<-morphology_LDB.S$CA$eig
ev1_LDB[ev1_LDB>mean(ev1_LDB)]
evplot(ev1_LDB)

biplot(morphology_LDB.S, choices=c(1,2), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2)) #angles reflect corelations
biplot(morphology_LDB.S, choices=c(1,3), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2)) #angles reflect corelations
biplot(morphology_LDB.S, choices=c(2,3), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2)) #angles reflect corelations

# colors <- c("grey80","grey20","firebrick","gold","green","blue")
# colors <- c("grey20","firebrick","gold","green","blue")
colo <- grey.colors(7, start = 0, end = 0.6, gamma = 2.2, alpha = NULL)
colors <- c(colo[1],colo[7],colo[5:4],colo[6],colo[3:2])
# shapes <- c(3,0,1,2,5)
shapes <- c(10,13,1,2,4,8,3)
colors_LBD <- c(colo[7],colo[3],colo[1]) 
shapes_LBD <- c(shapes[3:4],shapes[2])

gg1_LDB<-ggplot(morphology_trimmed_for_PCA_LDB, aes(morphology_LDB.PCA[,1],morphology_LDB.PCA[,2], color = seed_category, shape=seed_category))+
  geom_point(size = 2, stroke = 1.125) +
  stat_ellipse(segments = 102) +
  scale_color_manual(values = colors_LBD) +
  scale_shape_manual(values = shapes_LBD) +
  labs(y="PC2", x= "PC1") +
  theme_minimal()
gg2_LDB<-ggplot(morphology_trimmed_for_PCA_LDB, aes(morphology_LDB.PCA[,1],morphology_LDB.PCA[,3], color = seed_category, shape=seed_category))+
  geom_point(size = 2, stroke = 1.125) +
  stat_ellipse(segments = 102) +
  scale_color_manual(values = colors_LBD) +
  scale_shape_manual(values = shapes_LBD) +
  labs(y="PC3", x= "PC1") +
  theme_minimal()
gg3_LDB<-ggplot(morphology_trimmed_for_PCA_LDB, aes(morphology_LDB.PCA[,2],morphology_LDB.PCA[,3], color = seed_category, shape=seed_category))+
  geom_point(size = 2, stroke = 1.125) +
  stat_ellipse(segments = 102) +
  scale_color_manual(values = colors_LBD) +
  scale_shape_manual(values = shapes_LBD) +
...
# just plots
morphology_2_LDB <- morphology_LDB  %>%
  mutate(seed_category = ifelse(species == "berlandieri", color, species)) %>%  
  # mutate(seed_category = case_when(
  #   seed_category == "lightred" ~ "red",
  #   seed_category == "darkred"  ~ "red",
...
Z<-cbind(morphology_LDB$outer_epiderm_mean,morphology$inner_epiderm_mean,morphology$pericarp_mean,morphology$volume,morphology$embryo_volume,morphology$perisperm_volume,morphology$length,morphology$width,morphology$height)
...
#EVplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_α3_LBD.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,2))
n <- length(ev1_LDB)
...
bsm$p <- 100*bsm$p/n
barplot(ev1_LDB, main="Eigenvalues", col="grey70", las=2)
abline(h=mean(ev1_LDB), col="grey20", lty = "dashed")
legend("topright", "Average eigenvalue", lwd=1, col="grey20", bty="n", lty = "dashed")
barplot(t(cbind(100*ev1_LDB/sum(ev1_LDB), bsm$p[n:1])), beside=TRUE, main="% variation", col=c("grey70","grey20"), las=2)
legend("topright", c("% eigenvalue", "Broken-stick model"),	pch=15, col=c("grey70","grey20"), bty="n")
dev.off()


#biplots
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_β3_LBD.png",w=width*3,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,3))
biplot(morphology_LDB.S, choices=c(1,2), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2))
biplot(morphology_LDB.S, choices=c(1,3), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2))
biplot(morphology_LDB.S, choices=c(2,3), scaling = 2, main="", display="species", col = "black", ylim = c(-2,2), xlim = c(-2,2))
dev.off()


#corrmatrix
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_γ3_LBD.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,1))
Z_LBD<-cbind(morphology_LDB$outer_epiderm_mean,morphology$inner_epiderm_mean,morphology$pericarp_mean,morphology$volume,morphology$embryo_volume,morphology$perisperm_volume,morphology$length,morphology$width,morphology$height)
colnames(Z_LBD)<-c("outer_epiderm","inner_epiderm","pericarp","volume","embryo_volume","perisperm","length","width","height")
pairs(Z_LBD, lower.panel=panel.smooth2,upper.panel=panel.cor,diag.panel=panel.hist)
dev.off()

# nonPCAplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_δ3_LBD.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
gg4_LDB
dev.off()

# PCAplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_ε3_LBD.png",w=width*3,h=height, res=300, units = "in")
grid::grid.newpage()
...
# print(gg2,vp=v2) 
# print(gg3,vp=v3) 
ggpubr::ggarrange(gg1_LDB,gg2_LDB,gg3_LDB, ncol = 3, common.legend = TRUE)
dev.off()
...
```{r lbd_stats_tests}
manova(cbind(morphology_LDB$outer_epiderm_mean,morphology_LDB$volume) ~ morphology_LDB$seed_category) %>%
  summary()

lm(formula = outer_epiderm_mean + volume + length + width + height + inner_epiderm_mean + pericarp_mean + embryo_volume ~ seed_category, data = morphology_LDB) %>% summary()
aov(formula = outer_epiderm_mean + volume + length + width + height + inner_epiderm_mean + pericarp_mean + embryo_volume ~ seed_category, data = morphology_LDB) %>% TukeyHSD()

lm(formula = outer_epiderm_mean + volume ~ seed_category, data = morphology_LDB) %>% summary()
aov(formula = outer_epiderm_mean + volume ~ seed_category, data = morphology_LDB) %>% TukeyHSD()

lm(formula = outer_epiderm_mean ~ seed_category, data = morphology_LDB) %>% summary()
aov(formula = outer_epiderm_mean ~ seed_category, data = morphology_LDB) %>% TukeyHSD()

lm(formula = volume ~ seed_category, data = morphology_LDB) %>% summary()
aov(formula = volume ~ seed_category, data = morphology_LDB) %>% TukeyHSD()
```
...
biplot(morphology.S, choices=c(1,2), scaling = 2, main="", display="species", col = "black") #angles reflect corelations
biplot(morphology.S, choices=c(1,3), scaling = 2, main="", display="species", col = "black") #angles reflect corelations
biplot(morphology.S, choices=c(2,3), scaling = 2, main="", display="species", col = "black") #angles reflect corelations

# colors <- c("grey80","grey20","firebrick","gold","green","blue")
# colors <- c("grey20","firebrick","gold","green","blue")
colors <- grey.colors(7, start = 0, end = 0.6, gamma = 2.2, alpha = NULL)
colors <- c(colors[1],colors[7],colors[6],colors[3:2],colors[5:4])
# shapes <- c(3,0,1,2,5)
shapes <- c(10,13,4,8,3,1,2)
...
#EVplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_α3.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,2))
n <- length(ev1)
bsm <- data.frame(j=seq(1:n), p=0)
...
#biplots
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_β3.png",w=width*3,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,3))
biplot(morphology.S, choices=c(1,2), scaling = 2, main="", display="species", col = "black")
biplot(morphology.S, choices=c(1,3), scaling = 2, main="", display="species", col = "black")
biplot(morphology.S, choices=c(2,3), scaling = 2, main="", display="species", col = "black")
dev.off()


#corrmatrix
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_γ3.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
par(mfrow=c(1,1))
Z<-cbind(morphology$outer_epiderm_mean,morphology$inner_epiderm_mean,morphology$pericarp_mean,morphology$volume,morphology$embryo_volume,morphology$perisperm_volume,morphology$length,morphology$width,morphology$height)
colnames(Z)<-c("outer_epiderm","inner_epiderm","pericarp","volume","embryo_volume","perisperm","length","width","height")
...
# nonPCAplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_δ3.png",w=width,h=height, res=300, units = "in")
grid::grid.newpage()
gg4
dev.off()

# PCAplot
png(file="/Users/aperium/Dropbox/Projects/Chenopodium/Drafts/Thesis/figures/pca_ε3.png",w=width*3,h=height, res=300, units = "in")
grid::grid.newpage()
# v1<-viewport(width = 1/3, height = 1, x = 1/6, y = 0.5)
# v2<-viewport(width = 1/3, height = 1, x = 1/2, y = 0.5)
# v3<-viewport(width = 1/3, height = 1, x = 5/6, y = 0.5)
